...
public class ShineGrassBlock extends SpreadableBlock implements Fertilizable {
    public ShineGrassBlock(Settings settings) {
        super(settings);
    }
    ...
    public void grow(ServerWorld world, Random random, BlockPos pos, BlockState state) {
        ...
        BlockState blockState = ModBlocks.SHINE_GRASS.getDefaultState();
    ...
    private static boolean canSpread(BlockState state, WorldView world, BlockPos pos) {
        BlockPos blockPos = pos.up();
        return ShineGrassBlock.canSurvive(state, world, pos) && !world.getFluidState(blockPos).isIn(FluidTags.WATER);
    }

    @Override
    public void randomTick(BlockState state, ServerWorld world, BlockPos pos, Random random) {
        if (!ShineGrassBlock.canSurvive(state, world, pos)) {
            world.setBlockState(pos, ModBlocks.SHINE_DIRT.getDefaultState());
            return;
        }
        if (world.getLightLevel(pos.up()) >= 9) {
            ...
            for (int i = 0; i < 10; ++i) {
                ...
                if (!world.getBlockState(blockPos).isOf(ModBlocks.SHINE_DIRT) || !ShineGrassBlock.canSpread(blockState, world, blockPos)) continue;
